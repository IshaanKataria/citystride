...
Every design, language, and scoring decision follows one rule: the score is a positive signal, never a deficit signal. High score equals "this street is doing the thing well." Low score equals "less of what you asked for right now."

This means:

- Color gradient runs vibrant-to-dim, never red/warning.
- Language is descriptive ("Lit and Lively"), never judgmental ("avoid this street").
...
- Click any segment: pinned card (bottom-left) showing street name, composite score, per-metric breakdown (raw value plus normalized 0-1 bar), confidence flags where applicable.
...
F5: 3-Route Generation and Display
...
F6: Route Freezing and Recompute
...
- 24hr pulse autoplay on the time slider.
- MCP server with 3 tools: plan_walk, describe_segment, find_amenity_nearby.
- Working "Open in Google Maps" link (if not done as freebie).
- Live-during-drag slider recolor.
...
- plan_walk: input from, to, time. Output: 3 routes with per-segment metadata.
- describe_segment: input point or address, optional time. Output: composite score + raw metrics.
- find_amenity_nearby: input point, amenity type (transit, drinking fountain, public toilet), radius. Output: nearby amenities.
...
13. Risk Mitigation and Cut Paths
...
- All four demo beats execute without failure.
- A judge can ask "why is this street scored higher?" and the inspector card provides a specific, evidence-based answer.
- Time slider produces visible, meaningful changes to the map.
- Routes are visibly distinct and the recommended route is defensibly the best-scored option.
- The positive-framing principle holds: no red, no "avoid," no stigma anywhere in the UI or AI output.
...
```
citystride/
  bake/      one-shot ingestion + graph build
  runtime/   Express API server + MCP server (stretch)
  web/       Vite + React + MapLibre SPA
...
cd bake && npm install && npm run build
```
